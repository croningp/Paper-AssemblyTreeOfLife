...
size_t findIDsIndexInX0(strType& inchi, std::vector<strType>& x0) {
...
int tryGunshot(intVec& tryThis, intVec2D& target, intVec& linkedTo) {
	...
	// case 4 is: tryThis embraces one or more target element
	// case 5 is: else, on action is needed later
	if (jFound == 0 && kFound != -1) { // kFound actually always != -1
		for (int j = jFound + 1; j < target[iFound].size(); ++j) {
			Found = false;
			for (int k = kFound + 1; k < tryThis.size(); ++k) {
				if (target[iFound][j] == tryThis[k]) {
					linkedTo[k] = iFound;
					Found = true;
					break;
				}
		...
		// check rest of tryThis element
		for (int k = kFound + 1; k < tryThis.size(); ++k) {
			if (linkedTo[k] == -1) {
				// --- check every element in tryThis

				for (int i = 0; i < target.size(); ++i) {
					// check if target[i] is embraced
					if (target[i][0] == tryThis[k]) {
						linkedTo[k] = i;
						for (int j = 1; j < target[i].size(); ++j) {
							Found = false;
							for (int h = k + 1; h < tryThis.size(); ++h) {
								if (target[i][j] == tryThis[h]) {
									linkedTo[h] = i;
									Found = true;
									break;
								}
...
void filter(int Ntry, std::vector<strType> x0, std::vector<size_t> y0,
	...
	std::vector<strType> bondsTakenInchi{}; // corresponding to bondsTaken
	...
	while (nUnchanged < Ntry) {
		...
		strType thisInchi = x0[randPool[rand() % PoolSize]];
		intVec2D & IDs = xID0[thisInchi];
		// 1 fragment is used for twice
		for (int vv = 0; vv < 2; ++vv) {
			...
			while (true) {
				...
				if (whichCase == 2) {// totally covered
					...
					bondsTakenInchi.push_back(thisInchi);
					nUnchanged = 0;
					break;
				}
				else if (whichCase == 4) { // tryThis embraces one or more bondsTaken
					size_t newFragNbond = (*it).size();// (*it) = newFrag
					for (size_t i = 0; i < xNbond.size(); ++i) {
						if (xNbond[i] == newFragNbond && thisInchi == x0[i]) { // found
							...
							for (size_t j = 0; j < linkedTo.size() - 1; ++j) {
								if (linkedTo[j] != -1
									...
									bondsTakenInchi.erase(bondsTakenInchi.begin() + linkedTo[j]);
								}
							}
							if (linkedTo.back() != -1) {
								bondsTaken.erase(bondsTaken.begin() + linkedTo.back());
								bondsTakenInchi.erase(bondsTakenInchi.begin() + linkedTo.back());
							}
							// add the new fragment
							bondsTaken.push_back(*it);// (*it) = newFrag
							bondsTakenInchi.push_back(thisInchi);
				...
				//else { // whichCase == 3 // should not be 5
				...
				//		}
				//	}
				//	size_t newFragNbond = (newFrag).size();// (*it) = newFrag
				//	std::sort(newFrag.begin(), newFrag.end());
				//	for (size_t i = 0; i < xNbond.size(); ++i) {
				//		if (xNbond[i] == newFragNbond) {
				//			if (!isVecInVec2D(newFrag, xID0[x0[i]])) {// (*it) = newFrag
				...
				//				}
				//				// add the new fragment
				//				bondsTaken.push_back(newFrag);// (*it) = newFrag
				//				bondsTakenInchi.push_back(thisInchi);
				//				newFragAttached = true;
				//				nUnchanged = 0;
				//				break;
	...
	for (size_t j = 0; j < bondsTaken.size(); ++j) {
		intVec& frag = bondsTaken[j];
		for (int i = 0; i < x0.size(); ++i) {
			if (bondsTakenInchi[j] == x0[i]) { // stronger than if (isVecInVec2D(frag, xID0[x0[i]]))
				...
				if (std::find(resXid.begin(), resXid.end(), i) == resXid.end()) {
					...
					if (dupXidFound == dupXid.end()) {
						...
						dupInchi.push_back(bondsTakenInchi[j]);
					}
					else {
...
std::vector<std::tuple<strType, strType, unsigned long long int>> 
MOL_BOND::cut_and_split() {
...
PATHSINGLE MOL_BOND::get1pathway(FRAGHIST& fraghist, int Nmol, int Ntry_freeze) {
	PATHSINGLE path{};
	if (fraghist.y.size() == 0) {
		path.PAindex = static_cast<int>(Bonds.size()) - Nmol;
		return path;
	}
	...
	while (true) {
		filter(Ntry, x, y, xID, // input parameters
			x, y, xID,
			dupN, dupIDs, dupInchi);
		for (size_t i = 0; i < dupN.size(); ++i) {
			size_t idx_x0 = findIDsIndexInX0(dupInchi[i], x0);
		...
		//if (dupN.empty() || x.empty()) // ------??? this may better but slower
		//	break;
		if (x.empty())
			break;
	}
	...
	size_t paindex = Bonds.size() - Nmol;
...
void MOL_BOND::getMinPathways(std::unordered_map<strType, intVec>& TreeRaw,
	PATHWAY_ASSEMBLY& PA, FRAGHIST& fraghist, int Nmol, 
	int NtryPath, int Ntry_freeze) {

	PA.trivial_index = static_cast<int>(Bonds.size()) - Nmol;

	for (int i = 0; i < NtryPath; ++i) {
		PATHSINGLE path = get1pathway(fraghist, Nmol, Ntry_freeze);
		// record into TreeRaw, for later hierarchy
...
/* =========================================== */
// generate the histItem of the original molecule (shortcut)
std::vector<std::tuple<strType, strType, unsigned long long int>>
MOL_BOND::histItemOrigMol() {
	intVec ids{};
	for (auto& bond : Bonds) {
		ids.push_back(bond.first);
	}
	std::vector<std::tuple<strType, strType, unsigned long long int>> histItem{};
	histItem.push_back(std::make_tuple(inchi, getIdentifierStr(ids), 1));
	return histItem;
}