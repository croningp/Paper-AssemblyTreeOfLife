...
void FRAGHIST::getHistOverall(MYINT totalM, long double z) {
	...
	std::vector<size_t> y_size_orig0{};
	xID_orig.clear();
	...
	for (auto& it : histOrig) {// go through every fragment
		...
		y_size_orig0.push_back(idTemp.size());
	}
	...
	// ------
	std::vector<std::pair<size_t, size_t> > order(y_size_orig0.size());
	size_t n = 0;
	for (auto it = y_size_orig0.begin(); it != y_size_orig0.end(); ++it, ++n)
		order[n] = std::make_pair(n, *it);
	std::sort(order.begin(), order.end(),
		[](auto& a, auto& b) { return a.second > b.second; });
	...
	for (auto& i : order) {
		x_orig.push_back(x_orig0[i.first]);
		y_mean_orig.push_back(static_cast<float>(y_size_orig0[i.first]));
		y_err_orig.push_back(0);
	}
}
